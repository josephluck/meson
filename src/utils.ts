...
export function hasVNodeChanged(
  ...
  const vNodeTypeHasChanged =
    isVNode(newVNode) &&
    isVNode(oldVNode) &&
    (newVNode as Types.VNode).type !== (oldVNode as Types.VNode).type
  const hasVNodeTurnedToComponent =
    (isVNode(newVNode) && isComponent(oldVNode)) ||
    (isVNode(oldVNode) && isComponent(newVNode))
  return (
    typeHasChanged ||
    stringHasChanged ||
    numberHasChanged ||
    vNodeTypeHasChanged ||
    hasVNodeTurnedToComponent
  )
}

export function hasComponentChanged(
  newComponent: Types.ValidVNode,
  oldComponent: Types.ValidVNode,
): boolean {
  return (
    isComponent(newComponent) &&
    isComponent(oldComponent) &&
    shouldComponentReplace(newComponent)
  )
}
...
export function lifecycle(
  ...
  if (isComponent(vNode) && (vNode as Types.Component)[method]) {
    if (method === 'onAfterMount' || method === 'onBeforeUnmount') {
      ;(vNode as Types.Component)[method](
        $node,
        (vNode as Types.Component).state,
        (vNode as Types.Component)._update,
      )
    } else {
      ;(vNode as Types.Component)[method](
        (vNode as Types.Component).state,
