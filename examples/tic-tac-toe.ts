...
function getAllColumns(board: Board): Board {
  return Array.from({ length: board.length }).map((_, columnIndex) =>
    board.reduce(
      (prev, row) => {
...
function isWon(vertice: Square[]): boolean {
  return (
    vertice.every(square => square === 'x') ||
    vertice.every(square => square === 'o')
  )
}
...
function createEmptyBoard(length): Board {
  return Array.from({ length }).map(() =>
    Array.from({ length }).map(() => '' as Square),
  )
}
...
const renderSquare = (
  onclick: (rowIndex: number, columnIndex: number) => any,
  rowIndex: number,
) => (square: Square, columnIndex: number) => {
  return h(
    'div',
    {
      style:
        'width: 10px; height: 10px; display: inline-block; margin: 5px; border: solid 1px black;',
...
const renderRow = (onclick: (rowIndex: number, columnIndex: number) => any) => (
  row: Row,
  rowIndex: number,
) => {
...
const view: View<State> = (state = defaultState, update) => {
  ...
  const renderWinner = () =>
    state.winner ? `Player ${state.winner} won!` : null
